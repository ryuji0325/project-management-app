...
class _DashboardPageState extends State<DashboardPage> with SingleTickerProviderStateMixin {
  ...
  void _showEditProjectDialog(String projectId, Map<String, dynamic> data) {
    ...
    final budgetCtrl = TextEditingController(text: (data['budget'] ?? '0').toString());
    String statusVal = (data['status'] ?? 'Active').toString();
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (dialogContext, setStateDialog) => AlertDialog(
          ...
          shape: RoundedRectangleBorder(
            ...
            side: const BorderSide(color: kCardGlassBorder),
          ),
          title: Text(
            'Edit Project Details',
            style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.w700, color: kTextPrimary),
          ),
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  style: const TextStyle(color: kTextPrimary),
                  decoration: const InputDecoration(labelText: 'Client / Department'),
                ),
                const SizedBox(height: 16),
                TextField(
                  ...
                  style: const TextStyle(color: kTextPrimary),
                  decoration: const InputDecoration(labelText: 'Allocated Budget (RM)'),
                ),
                const SizedBox(height: 16),
                Text(
                  'Project Status',
                ...
                DropdownButtonFormField<String>(
                  value: ['Active', 'Completed'].contains(statusVal) ? statusVal : 'Active',
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                final double? budgetVal = double.tryParse(budgetCtrl.text);
                await FirebaseFirestore.instance.collection('projects').doc(projectId).update({
                  'projectName': nameCtrl.text.trim(),
                  'client': clientCtrl.text.trim(),
                  if (budgetVal != null) 'budget': budgetVal,
                  'status': statusVal,
                });
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    'Comprehensive financial allocation and portfolio summary across $activeCount active projects out of $totalCount total projects. Export full audit report or view financial analytics.',
                        ...
                        Row(
                          children: [
                            Column(
                              ...
                              children: [
                                Text(
                                  "RM ${(data['budget'] ?? 0.0).toStringAsFixed(0)}",
                                  style: GoogleFonts.plusJakartaSans(fontSize: 14, fontWeight: FontWeight.w700, color: kAccentColor),
                                ),
                                const SizedBox(height: 4),
                                Container(
                                  padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                                  decoration: BoxDecoration(
                                    color: (isActive ? kAccentColor : Colors.blue).withValues(alpha: 0.12),
      ...
      child: Row(
        ...
        children: [
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('ACTIVE PROJECTS', style: GoogleFonts.plusJakartaSans(fontSize: 11, fontWeight: FontWeight.w700, color: kTextSecondary, letterSpacing: 1.0)),
              const SizedBox(height: 6),
              Text('$active / $total Active', style: GoogleFonts.plusJakartaSans(fontSize: 22, fontWeight: FontWeight.w800, color: kTextPrimary)),
              const SizedBox(height: 6),
              Text('Portfolio status monitoring', style: GoogleFonts.plusJakartaSans(fontSize: 11, color: kTextSecondary)),
            ],
          ),
          Container(
            padding: const EdgeInsets.all(12),
            decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.05), borderRadius: BorderRadius.circular(14)),
            child: const Icon(Icons.folder_special_rounded, color: kAccentColor, size: 22),
          )
        ],
      ),
    );
